...
/* Parse the given jbooter app dir and return a JDLObject */
function parseFromDir(dir) {
  ...
  const jdl = Parser.parseServerOptions(Reader.readEntityJSON(`${dir}/.yo-rc.json`)['generator-jbooter']);
  const entityDir = `${dir}/.jbooter`;
  let isJbooterDirectory = false;
  try {
    isJbooterDirectory = fs.statSync(entityDir).isDirectory();
  } catch (error) {
    // .jbooter dir doesn't exist'
    return jdl;
  }
  if (!isJbooterDirectory) {
    throw new buildException(
      exceptions.WrongDir,
