...
describe('::exportToJDL', () => {
  ...
  describe('when passing valid arguments', () => {
    describe('when exporting json to entity JDL', () => {
      const jdl = parseFromDir('./test/test_files/jbooter_app');
      Exporter.exportToJDL(jdl);
      const input = JDLReader.parseFromFiles(['./jbooter-jdl.jh']);
      ...
      for (let entityName of ['Country', 'Department', 'Employee', 'Job', 'JobHistory', 'Location', 'Region', 'Task']) {
        previousEntities[entityName] = readEntityJSON('./test/test_files/jbooter_app/.jbooter/' + entityName + '.json');
